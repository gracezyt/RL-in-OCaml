...
(* Returns an index of the list of actions corresponding to the epsilon greedy exploration method *)
...
let rec train (ep_length:int) (env:env_state) (curr_pos:pos) (q_table) (cum_reward: float) (curr_action_idx:int)=
  (* Takes the action specified by epsilon greedy and updates the Q-Table *)
  let update (env:env_state) = 
    (* Uses the action to take one step in the environment *)
    let curr_action = List.nth action_list curr_action_idx in
    let next_state, reward, new_env = move (curr_pos, curr_action) (env) in 
    let next_x, next_y = next_state in 

    (* Gets the next action as required for SARSA *)
    ...
    (* Computes the update (aka the element that needs to be changed in the q-table) *)
  ...
  (* Terminates episode if we've reached the specified number of timesteps *)
  if ep_length == 0 then q_table, cum_reward
  else 
    (* Recursively calls train *)
